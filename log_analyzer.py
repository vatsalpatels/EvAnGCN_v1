...
eval_k = 44 # to compute metrics @K (for instance precision@1000)
print_params = True # Print the parameters of each simulation
##### End parameters ######

if 'elliptic' in filename or 'ether' in filename or 'reddit' in filename or ('bitcoin' in filename and 'edge' in filename):
...
try:
    ...
    use_latest_ep = True
    try:
        print('Values at best Valid Epoch (%d) for target class: TRAIN Precision %0.4f - Recall %0.4f - F1 %0.4f' % (best_ep, prec_cl['TRAIN'][cl_to_plot_id][best_ep],rec_cl['TRAIN'][cl_to_plot_id][best_ep],f1_cl['TRAIN'][cl_to_plot_id][best_ep]))
        print('Values at best Valid Epoch (%d) micro-AVG: TRAIN Precision %0.4f - Recall %0.4f - F1 %0.4f' % (best_ep, prec['TRAIN'][best_ep],rec['TRAIN'][best_ep],f1['TRAIN'][best_ep]))
        res_map['precision'] = prec_cl['TRAIN'][cl_to_plot_id][best_ep]
        res_map['recall'] = rec_cl['TRAIN'][cl_to_plot_id][best_ep]
        res_map['F1'] = f1_cl['TRAIN'][cl_to_plot_id][best_ep]
        res_map['AVG-precision'] = prec['TRAIN'][best_ep]
        res_map['AVG-recall'] = rec['TRAIN'][best_ep]
        res_map['AVG-F1'] = f1['TRAIN'][best_ep]
    except:
    ...
    try:
        print('Values at best Valid Epoch (%d) for target class@%d: TRAIN Precision %0.4f - Recall %0.4f - F1 %0.4f' % (best_ep, eval_k, prec_at_k_cl['TRAIN'][cl_to_plot_id][best_ep],rec_at_k_cl['TRAIN'][cl_to_plot_id][best_ep],f1_at_k_cl['TRAIN'][cl_to_plot_id][best_ep]))
        res_map['precision@'+str(eval_k)] = prec_at_k_cl['TRAIN'][cl_to_plot_id][best_ep]
        res_map['recall@'+str(eval_k)] = rec_at_k_cl['TRAIN'][cl_to_plot_id][best_ep]
        res_map['F1@'+str(eval_k)] = f1_at_k_cl['TRAIN'][cl_to_plot_id][best_ep]

        print('Values at best Valid Epoch (%d) micro-AVG@%d: TRAIN Precision %0.4f - Recall %0.4f - F1 %0.4f' % (best_ep, eval_k, prec_at_k['TRAIN'][best_ep],rec_at_k['TRAIN'][best_ep],f1_at_k['TRAIN'][best_ep]))
        res_map['AVG-precision@'+str(eval_k)] = prec_at_k['TRAIN'][best_ep]
        res_map['AVG-recall@'+str(eval_k)] = rec_at_k['TRAIN'][best_ep]
        res_map['AVG-F1@'+str(eval_k)] = f1_at_k['TRAIN'][best_ep]
