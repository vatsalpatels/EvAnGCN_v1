...
class ethereum_dataset():
    def __init__(self,args):
        ...
        args.ethereum_args = u.Namespace(args.ethereum_args)

        #build edge data structure
        edges = self.load_edges(args.ethereum_args)

        edges = self.make_contigous_node_ids(edges)
        num_nodes = edges[:,[self.ecols.FromNodeId,
                            self.ecols.ToNodeId]].unique().size(0)

        timesteps = u.aggregate_by_time(edges[:,self.ecols.TimeStep],args.ethereum_args.aggr_time)
    ...
    def load_edges(self,ethereum_args):
        file = os.path.join(ethereum_args.folder,ethereum_args.edges_file)
        with open(file) as f:
