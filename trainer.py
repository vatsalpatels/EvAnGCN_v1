...
class Trainer:
	...
	def load_checkpoint(self, filename, model):
		if os.path.isfile(filename):
			...
			self.logger.log_name("=> loaded checkpoint '{}' (epoch {})".format(filename, checkpoint['epoch']))
			return epoch
		else:
			self.logger.log_name("=> no checkpoint found at '{}'".format(filename))
			return 0
	...
	def train(self):
		...
		for e in range(self.args.num_epochs):
			...
			if len(self.splitter.test)>0 and eval_valid==best_eval_valid and e>self.args.eval_after_epochs:
				...
				# 	self.save_node_embs_csv(nodes_embs, self.splitter.train, '_train_nodeembs.csv.gz')
				# 	self.save_node_embs_csv(nodes_embs, self.splitter.dev, '_valid_nodeembs.csv.gz')
				# 	self.save_node_embs_csv(nodes_embs, self.splitter.test, '_test_nodeembs.csv.gz')
		#
		# now = datetime.datetime.now()
		# x = str(now.day) + "_" + now.strftime("%H") + "_" + now.strftime("%M") + "_" + now.strftime("%S")
	...
	def run_epoch(self, split, epoch, set_name, grad):
		...
		for s in split:
			...
			# print("SSSSS",s.label_sp['vals'].shape)

			predictions, nodes_embs, extract_predictions = self.predict(s.hist_adj_list,
												   s.hist_ndFeats_list,
												   s.label_sp['idx'],
												   s.node_mask_list)
			probs = torch.softmax(predictions, dim=1)[:, 1]
			np.savez('Eval_Train' + '.npz', trainloss=probs.detach().numpy())
			# print("predictions", predictions)
			# print("nodes_embs", nodes_embs)

			loss = self.comp_loss(predictions,s.label_sp['vals'])  ##Causing Error
			# loss = binary_cross_entropy(predictions, s.label_sp['vals'])  ##Causing Error
			# print("LOSS", loss)
	...
	def predict(self,hist_adj_list,hist_ndFeats_list,node_indices,mask_list):
		...
		for i in range(1 +(node_indices.size(1)//predict_batch_size)):
			...
			# print("RAW Predication", predictions)
			gather_predictions.append(predictions)
		gather_predictions=torch.cat(gather_predictions, dim=0)
		extract_predictions = gather_predictions
		# print("GatherPredications", gather_predictions)
		return gather_predictions, nodes_embs, extract_predictions
	...
	def prepare_sample(self,sample):
		...
		for i,adj in enumerate(sample.hist_adj_list):
			...
			# print("Sample HistAdjList", sample.hist_adj_list[i])

			nodes = self.tasker.prepare_node_feats(sample.hist_ndFeats_list[i])
			# print("PREPARE SAMPLE NODES", nodes)
	...
	def save_node_embs_csv(self, nodes_embs, indexes, file_name):
		...
		for node_id in indexes:
			...
			print("CSVNODEEMBS", csv_node_embs)
