...
# def get_sp_adj(edges, weighted, time_window):
#     idx = edges['idx']
#     unique_timesteps = idx[:, ECOLS.time].unique()
#
#     for i in unique_timesteps:
#         subset = idx[:, ECOLS.time] == i
...
def get_sp_adj(edges, time,weighted,time_window):
    # print("time", time)
    # print("time-time_window", (time - 1))
    idx = edges['idx']
    # print("Original IDX", idx.shape)
    subset = idx[:,ECOLS.time] <= time
    # print("SUB1", subset)
    subset = subset * (idx[:,ECOLS.time] > (time - time_window))
    # print("SUB2", time_window)
    idx = edges['idx'][subset][:,[ECOLS.source, ECOLS.target]]
    # print("IDX1", idx)
    vals = edges['vals'][subset]
    # print("vals length", vals)
    out = torch.sparse_coo_tensor(idx.t(), vals)
    # print("OUT",out.shape)
